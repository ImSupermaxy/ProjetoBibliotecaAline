...
public class ConsoleUtils {
    ...
    public static Date lerData(Scanner scanner, String mensagem) {
        ...
        try {
            return Date.from(LocalDate.parse(texto, DATE_FORMATTER).atStartOfDay(ZoneId.systemDefault()).toInstant());
    ...
    // Lê um número inteiro com validação
    public static int lerInt(Scanner scanner, String mensagem, Predicate<Integer> formato) {
        int valor = 0;
        ...
        while (valueInvalid) {
            ...
            if (scanner.hasNextInt()) {
                ...
                if (formato.test(valor)) {
                    ...
                    System.out.println("Valor fora do formato esperado. Tente novamente.");
                }
            } else {
                System.out.println("Valor inválido. Digite um número inteiro.");
    ...
    // Lê uma linha não vazia com um formato específico
    public static String lerLinha(Scanner scanner, String mensagem, Predicate<String> formato) {
        System.out.print(mensagem + ": ");
        String linha = scanner.nextLine().trim();

        var linhaIsNotValid = linha.isEmpty() || !formato.test(linha);

        while (linhaIsNotValid) {
            System.out.println("Entrada inávlida. Digite novamente.");
            System.out.print(mensagem + ": ");
            linha = scanner.nextLine().trim();
            linhaIsNotValid = linha.isEmpty() || !formato.test(linha);
        }
        return linha;
    }
